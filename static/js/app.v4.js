...
document.addEventListener("DOMContentLoaded", () => {
  checkConfig();
  bindCategorySeg();
  // 打开页面就自动把上次截帧显示出来，避免用户点错旧「预览」
  showLatestImages({ silent: true });
});
...
function assetUrl(path) {
  ...
  // Windows 绝对路径兜底：只保留 images/... 段
  const imgIdx = p.toLowerCase().lastIndexOf("/images/");
  if (imgIdx >= 0) p = p.slice(imgIdx + 1); // images/...
...
async function showLatestImages(opts = {}) {
  ...
  try {
    ...
    renderResult({
      ...
      files: data.images,
      title: "配图预览（验证显示）",
      body: "下面是视频真实截帧。能看到大图就说明显示正常。完整画廊也可打开 /gallery",
      tags: ["红笔", "配图验证"],
      note: "",
    });
